...
export class MemoryAccessView {

    public static currentPanel: MemoryAccessView | undefined;
    ...
    public static activate(context: ExtensionContext) {
        ...
        context.subscriptions.push(
            commands.registerCommand('memoryAccessView.start', () => {
                const columnToShowIn = window.activeTextEditor
                    ? window.activeTextEditor.viewColumn
                    : undefined;

                if (MemoryAccessView.currentPanel) {
                    MemoryAccessView.currentPanel._panel.reveal(columnToShowIn);
                }
                else {
                    ...
                    MemoryAccessView.currentPanel = new MemoryAccessView(panel, uri);
                }
            })
        );
    }
    ...
    private dispose() {
        MemoryAccessView.currentPanel = undefined;
    ...
    private _getWebviewContent(webview: Webview, extensionUri: Uri) {
        const webviewUri = getUri(webview, extensionUri, ["out", "memoryAccessView.webview.js"]);
        const nonce = getNonce();
        const styleUri = getUri(webview, extensionUri, ["out", "memoryAccessView.css"]);
        ...
        <html lang="en">
            <head>
                ...
                <title>Memory Access View</title>
